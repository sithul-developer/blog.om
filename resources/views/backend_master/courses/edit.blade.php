        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card m-0 pt-3">
                    <div class="card-body">
                        <form  action="{{ url('/panel/dashboard/courses/update/' . $courses->id) }}" method="POST"
                                    ...
                                    <select class="form-select" name="category_id" id="validationCustom04" required="">
                                        ...
                                        @foreach ($categories as $item)
                                            <option value='{{ $item->id }}'
                                                {{ $item->id == $courses->category_id ? 'selected' : '' }}>
                                                {{ $item->name }} </option>
                                        @endforeach
...
<a
                                                href="{{ route('courses.index') }}" style="color:whitesmoke "> <span class="btn btn-secondary  btn-sm mx-2"> <i
